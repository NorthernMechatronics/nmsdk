...
portBASE_TYPE prvGpioCommand(char * pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString);
...
static void GpioHelpSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
	...
	if (strncmp(pcParameterString, "init", xParameterStringLength) == 0)
	{
		...
		strcat(pcWriteBuffer, "\t<pin number> is one of the available GPIO pins\r\n");
	}
	else if (strncmp(pcParameterString, "deinit", xParameterStringLength) == 0)
	{
	...
	else if (strncmp(pcParameterString, "toggle", xParameterStringLength) == 0)
	{
...
static void GpioInitSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
	const char *pcParameterString;
	portBASE_TYPE xParameterStringLength;
	...
	if (strncmp(pcParameterString, "output", xParameterStringLength) == 0)
	{
		...
		if (pcParameterString == NULL)
		{
			...
			strcat(pcWriteBuffer, "Configured all unreserved GPIO pins to output.\r\n");
		}
		else
		{
	...
	else if (strncmp(pcParameterString, "input", xParameterStringLength) == 0)
	{
		pcParameterString = FreeRTOS_CLIGetParameter(pcCommandString, 3, &xParameterStringLength);
		if (pcParameterString == NULL)
		{
			GpioInitInput(MAX_GPIO_PINS);
			strcat(pcWriteBuffer, "Configured all unreserved GPIO pins to input.\r\n");
		}
		else
		{
			uint32_t ui32Pin = atoi(pcParameterString);
...
static void GpioDeinitSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
	const char *pcParameterString;
	portBASE_TYPE xParameterStringLength;
...
static void GpioWriteSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
	const char *pcParameterString;
	portBASE_TYPE xParameterStringLength;
...
static void GpioReadSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
	const char *pcParameterString;
	portBASE_TYPE xParameterStringLength;
	...
	else if (strncmp(pcParameterString, "output", xParameterStringLength) == 0)
	{
...
static void GpioToggleSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
	const char *pcParameterString;
	portBASE_TYPE xParameterStringLength;
	uint32_t ui32Pin;
...
portBASE_TYPE prvGpioCommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
	...
	else if (strncmp(pcParameterString, "init", xParameterStringLength) == 0)
	{
