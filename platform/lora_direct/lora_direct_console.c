...
portBASE_TYPE prvLoRaCommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString);

const CLI_Command_Definition_t prvLoRaCommandDefinition =
{
	(const char * const) "lora",
	(const char * const)
		"lora:\tLoRa radio command interface.\r\n"
	,
	prvLoRaCommand,
	-1
};

static void LoRaHelpSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
	...
	if (pcParameterString == NULL)
	{
		...
		strcat(pcWriteBuffer, "  display  show/hide received payload content\r\n");
		strcat(pcWriteBuffer, "  help     show command details\r\n");
		strcat(pcWriteBuffer, "\r\n");
		strcat(pcWriteBuffer, "See 'lora help <command>' for command details.\r\n");
	...
	else if (strncmp(pcParameterString, "send", 4) == 0)
	{
		...
		strcat(pcWriteBuffer, "  freq  is in MHz, real scalar\r\n");
		strcat(pcWriteBuffer, "  power is in dBm, integer scalar\r\n\r\n");
		strcat(pcWriteBuffer, "Note: freq and power must be specified together.\r\n");
	}
...
static void LoRaInitSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
...
static void LoRaDeinitSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
...
static void LoRaResetSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
	lora_radio_reset(NULL);
	strcat(pcWriteBuffer, "LoRa radio reset\r\n");
}

static void LoRaTxcwSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
...
static void LoRaSendSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
	const char *pcParameterString = NULL, *pcTransmitPower = NULL;
	portBASE_TYPE xParameterStringLength;
	double f;
	...
	if (argc == 2)
	{
		lora_direct_send(lora_radio_frequency, lora_radio_power,
				(const uint8_t *)pcParameterString, xParameterStringLength);
	...
	else
	{
		...
		lora_direct_send(freq, power, (const uint8_t *)pcParameterString, xParameterStringLength);
...
static void LoRaRxSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
...
static void LoRaSetSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
	const char *pcParameterString = NULL;
	portBASE_TYPE xParameterStringLength;
	double f;
	...
	char *buffer = pcWriteBuffer + strlen(pcWriteBuffer);
	if (strncmp(pcParameterString, LORA_RADIO_FREQUENCY, xParameterStringLength) == 0)
	{
	...
	else if (strncmp(pcParameterString, LORA_RADIO_POWER, xParameterStringLength) == 0)
	{
		pcParameterString = FreeRTOS_CLIGetParameter(pcCommandString, 3, &xParameterStringLength);
	...
	else if (strncmp(pcParameterString, LORA_RADIO_SPREADING_FACTOR, xParameterStringLength) == 0)
	{
		pcParameterString = FreeRTOS_CLIGetParameter(pcCommandString, 3, &xParameterStringLength);
		...
		{
			gsLoRaModulationParameter.eSpreadingFactor = atoi(pcParameterString);
		}

		am_util_stdio_sprintf(buffer, "\r\nSpreading Factor: %d\r\n", gsLoRaModulationParameter.eSpreadingFactor);
	}
	else if (strncmp(pcParameterString, LORA_RADIO_BANDWIDTH, xParameterStringLength) == 0)
	{
		pcParameterString = FreeRTOS_CLIGetParameter(pcCommandString, 3, &xParameterStringLength);
		...
		else
		{
			...
			if (bandwidth == -1)
			{
				am_util_stdio_sprintf(buffer, "\r\nUnknown bandwidth specified.  Parameters unchanged.\r\n");
			}
			else
			{
				am_util_stdio_sprintf(buffer, "\r\nBandwidth: %d kHz\r\n", bandwidth);
	...
	else if (strncmp(pcParameterString, LORA_RADIO_CODING_RATE, xParameterStringLength) == 0)
	{
		pcParameterString = FreeRTOS_CLIGetParameter(pcCommandString, 3, &xParameterStringLength);
		...
		else
		{
			...
			if (redundancy == -1)
			{
				am_util_stdio_sprintf(buffer, "\r\nUnknown redundancy specified.  Parameters unchanged.\r\n");
			}
			else
			{
...
static void LoRaGetSubcommand(char *pcWriteBuffer, size_t xWriteBufferLen, const char *pcCommandString)
{
	const char *pcParameterString = NULL;
	portBASE_TYPE xParameterStringLength;
	...
	else if (strncmp(pcParameterString, LORA_RADIO_SYNCWORD, xParameterStringLength) == 0)
	{
		am_util_stdio_sprintf(buffer, "\r\nSync Word: 0x%08X\r\n", lora_radio_syncword);
	}
	else
